#pragma once

// DeepRockGalactic SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "FSD_Options_DisableMasterEQ_classes.hpp"

namespace SDK
{
//---------------------------------------------------------------------------
//Parameters
//---------------------------------------------------------------------------

// Function Options_DisableMasterEQ.Options_DisableMasterEQ_C.Construct
struct UOptions_DisableMasterEQ_C_Construct_Params
{
};

// Function Options_DisableMasterEQ.Options_DisableMasterEQ_C.UINeedsUpdate
struct UOptions_DisableMasterEQ_C_UINeedsUpdate_Params
{
};

// Function Options_DisableMasterEQ.Options_DisableMasterEQ_C.ShowOptions
struct UOptions_DisableMasterEQ_C_ShowOptions_Params
{
};

// Function Options_DisableMasterEQ.Options_DisableMasterEQ_C.BndEvt__Basic_CheckBox_K2Node_ComponentBoundEvent_15_OnCheckStateChanged__DelegateSignature
struct UOptions_DisableMasterEQ_C_BndEvt__Basic_CheckBox_K2Node_ComponentBoundEvent_15_OnCheckStateChanged__DelegateSignature_Params
{
	bool*                                              IsChecked;                                                // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

// Function Options_DisableMasterEQ.Options_DisableMasterEQ_C.ExecuteUbergraph_Options_DisableMasterEQ
struct UOptions_DisableMasterEQ_C_ExecuteUbergraph_Options_DisableMasterEQ_Params
{
	int*                                               EntryPoint;                                               // (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
